...
import type { ChatReference } from "@baindar/ui";
...
export const composerReferenceTags = (
...
function referenceColor(reference: MessageReference): string | undefined {
  if (reference.kind === "highlight" && reference.color) {
    return highlightColor(reference.color);
  }
  if (reference.kind === "passage") return color.status.info;
  if (reference.kind === "note") return color.slate[500];
  return color.wine[700];
}

function highlightColor(value: string): string | undefined {
  if (value === "pink") return color.highlight.pink;
  if (value === "yellow") return color.highlight.yellow;
  if (value === "green") return color.highlight.green;
  if (value === "blue") return color.highlight.blue;
  if (value === "purple") return color.highlight.purple;
  return undefined;
}
